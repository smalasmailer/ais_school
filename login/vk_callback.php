...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($input['access_token'])) {
        ...
        $user_info_url = "https://api.vk.com/method/users.get?access_token={$access_token}&v=5.131&scope=offline";
        ...
        if (isset($user_info_data['response'][0]['id'])) {
            ...
            $res = $conn->query("SELECT `login` FROM `users` WHERE `vk` = '$user_id'");
            if($res->num_rows>0){
                $login = $res->fetch_assoc()["login"];
                setcookie("acclogin", $login, time() + 3600, "/");
            }
        } else {
