...
if (checkTelegramAuthorization($auth_data)) {
    ...
    $res = $conn->query("SELECT `login` FROM `users` WHERE `tg` = '$user_id'");
    ...
    if($res->num_rows > 0) {
        ...
        if (isset($acclogin)) {
            ...
            $conn->query("UPDATE `users` SET `tg` = '$user_id' WHERE `login` = '$acclogin'"); // Убедитесь, что вы добавили условие для обновления
            header("Location: loginsuccess.php");
            exit();
        }
    }
